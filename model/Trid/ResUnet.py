...
import torch
from model.Trid.resnet import resnet34, resnet18, resnet50, resnet101, resnet152
import torch.nn.functional as F
from model.Trid.unet import UnetBlock
...
class ResUnet(nn.Module):
    ...
    def forward(self, input,phase=None):
        assert phase in ['train', 'test'], 'forward phase should be set'
        x, sfs = self.res(input,phase)
        x = F.relu(x)
...
if __name__ == "__main__":
    model = ResUnet(resnet='resnet34', num_classes=1, pretrained=False, mixstyle_layers=['layer1'], random_type='TriD')
    model.cuda().eval()
