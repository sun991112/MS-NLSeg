...
# 将 unet 目录的路径添加到 sys.path
sys.path.insert(0, os.path.abspath('/home/wy3atjlu/zhaozq/mount8t/subjects/multisite_medsam/'))

from model.unet import UNet

class RandConvModule(nn.Module):
    def __init__(self, net=None, kernel_size=3, in_channels=1, out_channels=1,
                 rand_bias=False,
                 mixing=False,
        ...
        if isinstance(kernel_size, collections.abc.Sequence) and len(kernel_size) == 1:
...
class UNet_with_randconv(nn.Module):
    def __init__(self):
        super(UNet_with_randconv, self).__init__()
        self.UNet=UNet()
        self.randconv=RandConvModule()
    def forward(self, input):
        input01=input[:, 0:1, :, :]
        input12=input[:, 1:2, :, :]
        zero_img = torch.zeros_like(input12)  # [B, 1, 64, 64]

        self.randconv.randomize()
        input1 = self.randconv(input01)
        input4 = self.randconv(input12)
        self.randconv.randomize()
        input2 = self.randconv(input01)
        input5 = self.randconv(input12)
        self.randconv.randomize()
        input3=self.randconv(input01)
        input6 = self.randconv(input12)
        randconv1=torch.cat([input1,input4,zero_img],dim=1  )
        randconv2 = torch.cat([input2, input5, zero_img], dim=1)
        randconv3 = torch.cat([input3, input6, zero_img], dim=1)
        output1=self.UNet(randconv1)
        output2=self.UNet(randconv2)
        output3=self.UNet(randconv3)
        return output1, output2, output3
if __name__ == '__main__':
    from loss import RandConvLoss
    loss=RandConvLoss()
    model = UNet_with_randconv()
    data=torch.randn(5,3,512,512)
    mask=torch.ones(5,1,512,512)
    a,b,c =model(data)
